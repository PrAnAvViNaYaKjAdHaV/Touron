...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_API_URL}/api/request/upcomingtrips`)
      .then((res) => {
    ...
    axios
      .get(`${import.meta.env.VITE_API_URL}/api/request/currenttrips`)
      .then((res) => {
        console.log(res.data.count);
    ...
    axios
      .get(`${import.meta.env.VITE_API_URL}/api/request/cancelledtrips`)
      .then((res) => {
        console.log(res.data.count);
    ...
    axios
      .get(`${import.meta.env.VITE_API_URL}/api/request/completedtrips`)
      .then((res) => {
        console.log(res.data.count);
  ...
  return (
    <div className=" ml-80 flex flex-col gap-20 py-20 px-20 bg-stone-50 min-h-screen">
      <div className="flex justify-between bg-white rounded-xl shadow-md py-5 px-8">
        <StatisticsCard
        ...
        <table className="min-w-full bg-white shadow-xl rounded-2xl">
          ...
          <tbody className="text-gray-700">
            {requests.map((row, index) => (
              <tr key={row._id} className="text-left border-b border-stone-200">
