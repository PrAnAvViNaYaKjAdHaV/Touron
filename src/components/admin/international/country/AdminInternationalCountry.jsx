...
import AddInternationalCountryPopup from "../../../../layout/admin/international/country/AddInternationalCountryPopup";

const AdminInternationalCountry = () => {
  ...
  const handleDelete = (id) => {
    axios
      .post(`${import.meta.env.VITE_API_URL}/api/country/delete`, { id })
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        setInternationalCountry(response.data);
      })
      .catch((error) => {
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full bg-white shadow-xl rounded-2xl">
            ...
            <tbody className="text-gray-700">
              {internationalCountry.map((row, index) => (
                <tr
                  key={row._id}
                ...
                >
                  ...
                  <td className="py-3 px-5">
                    {row.aboutCountry?.length > 120
                      ? `${row.aboutCountry.slice(0, 120)}...`
                      : row.aboutCountry}
                  </td>
                  <td className="py-3 px-5">{row.weather}</td>
...
export default AdminInternationalCountry;
